...
- Ingestion is streaming-safe (no full-file loads): NDJSON files are read line-by-line and inserted with explicit column mapping; insertion order follows NDJSON file order (deterministic given the same input bytes).
