...
- `.ai/` audit trail requirements (prompts, sessions, ADRs)
- `.ai/time/time.csv` mandatory effort logging
...
────────────────────────────────────────────────────────────
0.7 .ai/time/time.csv (MANDATORY)
────────────────────────────────────────────────────────────

Maintain .ai/time/time.csv at repo root with columns:
...
- missing .ai/time/time.csv entry = incomplete work
...
────────────────────────────────────────────────────────────
0.9 Codex Audit Trail (.ai Directory) — MANDATORY
────────────────────────────────────────────────────────────

The repository MUST contain a top-level `.ai/` directory.
This directory is the **authoritative audit trail of Codex reasoning, prompts, and decisions**.

Rules:
- `.ai/` is committed to Git
- `.ai/` does NOT contain secrets or tokens
- `.ai/` is append-only in spirit; do not rewrite history
...
1) Prompts
   - Every substantial instruction or refinement given to Codex
     MUST be recorded in `.ai/prompts/`
...
2) Sessions
   ...
   - Session logs summarize:
     ...
     - decisions made
     - follow-ups created
   - Sessions must align with .ai/time/time.csv entries
...
4) Relationship to .ai/time/time.csv
   - .ai/time/time.csv tracks effort
   - `.ai/` tracks intent, reasoning, and decision-making
   - Both are required for a change to be considered complete

Failure to update `.ai/` for meaningful Codex-driven work
...
7. Update docs
8. Update .ai session logs / prompts / ADRs
9. Append .ai/time/time.csv
...
- create .ai/ directory with README.md
