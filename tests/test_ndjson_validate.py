...
class TestNdjsonValidate(unittest.TestCase):
    def test_schema_files_exist_for_all_streams(self) -> None:
        ...
        expected = {
            ...
            "imaging_studies.schema.json",
            "specimens.schema.json",
            "devices.schema.json",
    ...
    def test_validate_imaging_studies_requires_imaging_study_contract_fields(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            nd = root / "ndjson"
            nd.mkdir(parents=True, exist_ok=True)
            ...
            _write(
                nd / "imaging_studies.ndjson",
                '{"schema_version":2,"record_key":"k1","canonical_person_id":"p1","source":"fhir","source_file":"source/clinical/img.json","event_time":"2020-01-13T15:00:00Z","run_id":"r1","resource_type":"ImagingStudy","source_id":"ImagingStudy/img1","status":"available"}\n',
            )
            ...
            self.assertEqual(r.returncode, 1, msg=f"stdout={r.stdout}\nstderr={r.stderr}")
            self.assertIn("imaging_study_id", r.stderr)

    def test_validate_specimens_requires_specimen_contract_fields(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            nd = root / "ndjson"
            nd.mkdir(parents=True, exist_ok=True)
            ...
            _write(
                nd / "specimens.ndjson",
                '{"schema_version":2,"record_key":"k1","canonical_person_id":"p1","source":"fhir","source_file":"source/clinical/specimen.json","event_time":"2020-01-14T10:30:00Z","run_id":"r1","resource_type":"Specimen","source_id":"Specimen/sp1"}\n',
            )
            ...
            self.assertEqual(r.returncode, 1, msg=f"stdout={r.stdout}\nstderr={r.stderr}")
            self.assertIn("specimen_id", r.stderr)

    def test_validate_devices_requires_device_contract_fields(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            nd = root / "ndjson"
            nd.mkdir(parents=True, exist_ok=True)
            ...
            _write(
                nd / "devices.ndjson",
                '{"schema_version":2,"record_key":"k1","canonical_person_id":"p1","source":"fhir","source_file":"source/clinical/device.json","event_time":"","run_id":"r1","resource_type":"Device","source_id":"Device/dev1","status":"active"}\n',
            )
